...
namespace OnvifTests
{
    public class MediaClientTests
    {
        ...
        public async Task GetGuaranteedNumberOfVideoEncoderInstances()
        {
            MediaClient mc = getSubject();
            string token = await getVideoSourceConfigurationToken(mc);
            GetGuaranteedNumberOfVideoEncoderInstancesResponse getGuaranteedNumberOfVideoEncoderInstancesResponse = await mc.GetGuaranteedNumberOfVideoEncoderInstances(token);
            Assert.That(getGuaranteedNumberOfVideoEncoderInstancesResponse.TotalNumber, Is.GreaterThanOrEqualTo(0));
        }
