...
- [ ] For optional primitive value types (`bool`, `int`, `float`, etc.) that are mapped as XML attributes (`[XmlAttribute]`), use the `value + *Specified` pattern for XML serialization compatibility: keep the DTO value member non-nullable and add a paired `*Specified` member marked with `[XmlIgnore]`.
